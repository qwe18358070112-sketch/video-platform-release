...
set "LAUNCH_CONFIG=fixed_layout_programs\config.layout12.fullscreen.yaml"
...
if %ERRORLEVEL% EQU 0 if not defined VIDEO_PLATFORM_WINDOWS_REDIRECTED (
  if defined WINDOWS_RUNTIME_ROOT (
    ...
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%WINDOWS_RUNTIME_ROOT%\windows_bridge.ps1" -RepoPath "%WINDOWS_RUNTIME_ROOT%" -Action run -AllowAutoElevate --config "%LAUNCH_CONFIG%" --layout 12 --mode fullscreen
...
%PYTHON_CMD% app.py --run --config fixed_layout_programs\config.layout12.fullscreen.yaml --layout 12 --mode fullscreen
set "EXIT_CODE=!ERRORLEVEL!"
popd
if not "!EXIT_CODE!"=="0" (
  echo.
